...
class ConsoleApp( Frame ):
    ...
    def __init__( self, nets=None, parent=None, width=4 ):
        Frame.__init__( self, parent )
        #data structure
        self.netManager = Nets()
        self.netManager.addNet("init",Mininet(SingleSwitchTopo(),autoSetMacs=True))
        self.dataManager = DataManager
        self.netName = "init"
        self.netManager.runingNet("init")
        #CLI(self.netManager.nets["init"])
        ...
        for name in titles:
            nodes = getattr( self.netManager.nets[self.netName], name)
    ...
    def createConsoles( self, parent, nodes, width, title ):
        ...
        for node in nodes:
            info(type(node))
            console = Console( f, self.netManager.nets[self.netName], node, title=title )
    ...
    def createMenuBar(self):
        ...
        fileMenu.add_command(label='导入拓扑', command=None)
        fileMenu.add_command(label='导入拓扑', command=None)
